# Streamlit App for VayuSense - Simplified Version
...
def main():
    ...
    if app_mode == '🏠 Home':
        ...
        st.info("⚠️ ML models are currently being optimized for deployment. For now, we're showing sample predictions.")
        ...
        with col1:
            if st.button('🔮 Predict Emissions', type='primary'):
                # Sample predictions based on country
                base_emissions = {
                ...
                prediction = base_value * year_factor * np.random.uniform(0.9, 1.1)
                ...
                emissions = []
                for y in years:
                    factor = 1 + (y - 2024) * 0.02
                    emissions.append(base_value * factor)
